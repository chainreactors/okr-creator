...
- **改进模式** — 基于诊断选择的 2-4 个改进策略（来自 `references/patterns.md`）
- **KR 分解** — P0/P1 KR 的 weekly breakdown
...
2. 写入 SKILL.md
3. 创建初始 PROGRESS.md（所有 KR 进度 0%，状态 not_started，六维分数取自诊断，行动队列为空）
4. 读回验证
5. 已存在旧文件时，展示 diff 让用户确认再覆盖
...
4. **询问用户：** "是否部署 PR OKR 对齐检查？（推荐用于团队协作项目）"
   - 是 → 读取 `templates/okr-align-check.yml` 和 `templates/okr-align-prompt.md`，分别写入目标路径
   - 否 → 跳过
6. 如果目标路径已有同名文件，展示 diff 让用户确认再覆盖
7. 创建 `okr-review` label（如果 `gh` CLI 可用）：`gh label create "okr-review" --description "OKR daily review tracking" --color "0E8A16" 2>/dev/null || true`
